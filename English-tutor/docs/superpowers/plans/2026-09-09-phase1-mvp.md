...
**Interfaces:**
- Produces: `QUESTIONS: list[dict]` loaded from the JSON (20 items, ascending difficulty); `score_written(answers: list[int | None]) -> int`; `suggest_level(score: int, voice_hint: str | None = None) -> str` — bands A2:6–11, B1:12–16, B2:17–20; voice_hint (one of the levels) moves the result one band **only** when the score sits within 1 point of the boundary toward that band; `classify_voice(transcript: str, llm) -> str` — asks the LLM (see `english_tutor/llm/prompts.py` `VOICE_LEVEL_PROMPT`) via `llm.chat_json`, falls back to `None` on parse failure.
