...
def test_redeem_rejected_for_active_student(conn):
    code = invites.create_invite(conn)
    db.upsert_student(conn, 42, status="active", level="B1")
    assert invites.redeem(conn, code, 42) is False
    assert db.get_student(conn, 42)["status"] == "active"


def test_redeem_rejected_for_pending_student(conn):
    code = invites.create_invite(conn)
    db.upsert_student(conn, 42, status="pending")
    assert invites.redeem(conn, code, 42) is False
